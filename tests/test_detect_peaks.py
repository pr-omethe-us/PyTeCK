...
class TestDetectPeaks:
    ...
    def test_min_peak_distance(self):
        ...
        for idx, ind in enumerate(peaks):
            ...
            peaks_before = numpy.array([i for i in range(start, ind) if y[i] > y[ind]])
            peaks_after = numpy.array([i for i in range(ind + 1, end + 1) if y[i] > y[ind]])

            # # remove bad peaks if within minimum peak distance of previous peak
            if idx > 0:
                ind_prev = peaks[idx - 1]
                idel = numpy.zeros(peaks_before.size, dtype=bool)
                for i in range(peaks_before.size):
                    if (peaks_before[i] - ind_prev) <= mpd and y[ind_prev] > y[peaks_before[i]]:
                        idel[i] = 1
            ...
            if idx < len(peaks) - 1:
                ...
                idel = numpy.zeros(peaks_after.size, dtype=bool)
                for i in range(peaks_after.size):
                    if (ind_next - peaks_after[i]) <= mpd and y[ind_next] > y[peaks_after[i]]:
                        idel[i] = 1
    ...
    def test_sin_peaks_min_distance(self):
        ...
        rng = numpy.random.default_rng()
        y = numpy.sin(2 * numpy.pi * 5 * numpy.linspace(0, 1, 200)) + rng.uniform(size=200) / 5.0
    ...
    def test_valley_detect(self):
        ...
        for idx, ind in enumerate(valleys):
            ...
            valleys_before = numpy.array([i for i in range(start, ind) if y[i] < y[ind]])
            valleys_after = numpy.array([i for i in range(ind + 1, end + 1) if y[i] < y[ind]])
            ...
            if idx > 0:
                ...
                idel = numpy.zeros(valleys_before.size, dtype=bool)
                for i in range(valleys_before.size):
                    if (valleys_before[i] - ind_prev) <= mpd and y[ind_prev] < y[valleys_before[i]]:
                        idel[i] = 1
            ...
            if idx < len(valleys) - 1:
                ...
                idel = numpy.zeros(valleys_after.size, dtype=bool)
                for i in range(valleys_after.size):
                    if (ind_next - valleys_after[i]) <= mpd and y[ind_next] < y[valleys_after[i]]:
                        idel[i] = 1
